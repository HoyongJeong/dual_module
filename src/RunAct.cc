...
RunAct::RunAct(): G4UserRunAction()
{
	...
	// Creating ntuple
	AM -> CreateNtuple("dual_module", "Dual Module");
	AM -> CreateNtupleIColumn("eventID");   // Column ID = 0
	AM -> CreateNtupleIColumn("creProcID"); // Column ID = 1
	AM -> CreateNtupleIColumn("module");    // Column ID = 2
	AM -> CreateNtupleDColumn("timing");    // Column ID = 3
	AM -> CreateNtupleDColumn("energy");    // Column ID = 4
	AM -> FinishNtuple();
}
...
void RunAct::BeginOfRunAction(const G4Run* /*run*/)
{
	...
	G4String fileName = "dual_module_";
	fileName += sTime;
	fileName += ".root";
